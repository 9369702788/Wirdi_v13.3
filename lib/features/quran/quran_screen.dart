    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            tooltip: 'اختيار القارئ (${Reciters.byId(appSettings.reciterId).displayName})',
...
class _DownloadButtonState extends State<_DownloadButton> {
  ...
  Future<void> _startDownload() async {
    ...
    if (mounted) {
      ...
      if (!_cancelRequested) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('تم تحميل السورة للاستماع بدون اتصال')),
        );
      }
    }
  }
  ...
  Future<void> _deleteDownload() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('سيتم حذف الملفات الصوتية المحملة لهذه السورة.'),
        actions: [
  ...
  Widget build(BuildContext context) {
    if (_downloading) {
      return IconButton(
        tooltip: 'إيقاف التحميل ($_done/$_total)',
    ...
    return IconButton(
      tooltip: _downloaded ? 'محملة للاستماع بدون اتصال — اضغط للحذف' : 'تحميل السورة للاستماع بدون اتصال',
