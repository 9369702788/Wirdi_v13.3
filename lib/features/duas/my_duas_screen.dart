...
/// A personal, private collection of the user's own duas — free text
/// they write and save themselves. Distinct from the Azkar screen's
/// "الأدعية" tab (which surfaces pre-written duas from the Hisn Al
/// Muslim dataset): this is for the user's own words, kept local to
/// their device only, never shared or made social.
...
class _MyDuasScreenState extends State<MyDuasScreen> {
  ...
  Future<void> _addOrEditDua({_MyDua? existing}) async {
    ...
    final result = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: Text(existing == null ? 'دعاء جديد' : 'تعديل الدعاء'),
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(
                ...
                textAlign: TextAlign.right,
                maxLines: 5,
                decoration: const InputDecoration(labelText: 'نص الدعاء', alignLabelWithHint: true),
              ),
            ],
          ),
        ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('أدعيتي'), centerTitle: true),
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 12),
                        const Text('لم تُضِف أي دعاء بعد', style: TextStyle(color: AppColors.mutedText)),
                        const SizedBox(height: 8),
                        const Text('اضغط + لإضافة دعائك الخاص', style: TextStyle(color: AppColors.mutedText, fontSize: 12)),
                      ],
                    ),
                  ),
                )
